...
char *findenv(),*malloc(),*lalloc(),*mallocl();
long lseek();

#define INCNAME "LIB16"
...
struct use *ause;
	...
	datablock=mallocl(DATABLOCKSIZE);
	if(!datablock) nomem();
	datapoint=datablock;
	outbuff=mallocl(OUTBLOCKSIZE);
	if(!outbuff) nomem();
	outpoint=outbuff;
	outend=outbuff+OUTBLOCKSIZE;
	ioblock=lalloc(IOBLOCKSIZE);
	if(!ioblock) nomem();
