...
char paused;
	...
	remap2=lalloc(16384);
	if(!remap2) nomem();
	offscreen=lalloc(64000);
	if(!offscreen) nomem();
	offscreen2=lalloc(64000);
	if(!offscreen2) nomem();
	blobs=lalloc(MAXBLOBS*sizeof(struct blob));
	if(!blobs) nomem();

	puts("Fire demo by David Ashley (dash@netcom.com)\n");
	...
	puts("Right mouse button = ignite atmosphere\n");
	initmouse();
	while(!kr()) rand();
	rci();
	...
	clearmem(offscreen,320);
	clearmem(offscreen+64000-320,320);
	p=offscreen+320;
	i=64000-640;
	while(i--) *p++=rand()&3 ? 0 : 63;
	...
	while()
	{
		...
		if(!flash)
		{
			if(explodenum>96 && explodenum<160 && !(rand()&511) || mousebuttons&2)
				flash=60;
		} else --flash;
		...
		moveblobs();
		putblobs();
		bcopy(offscreen,offscreen2,64000);
		fire(offscreen2,offscreen,flash ? remap2 :remap);
/*this governs the fps rate...*/
		while((gtime()-stime) && frames*100/(gtime()-stime)>30)
		{
			while(!retrace());
			while(retrace());
		}
		bcopy(offscreen,video,64000);
		if(frames==20000)
		{
			frames=0;
			stime=gtime();
